...
export async function runWizard() {
  ...
  if (backends.includes('claude')) {
    const { claudeAuthMethod } = await inquirer.prompt([{
      type: 'list',
      name: 'claudeAuthMethod',
      message: 'Claude 认证方式:',
      choices: [
        { name: '官方账号登录 (claude login)', value: 'official' },
        { name: '三方 API Key', value: 'third-party' },
      ],
      default: (existing.ANTHROPIC_AUTH_TOKEN || existing.ANTHROPIC_BASE_URL) ? 'third-party' : 'official',
    }])
    if (claudeAuthMethod === 'third-party') {
      const ans = await inquirer.prompt([
        { type: 'input', name: 'ANTHROPIC_AUTH_TOKEN', message: 'Claude API Token:', default: existing.ANTHROPIC_AUTH_TOKEN || '' },
        { type: 'input', name: 'ANTHROPIC_BASE_URL', message: 'Claude API Base URL:', default: existing.ANTHROPIC_BASE_URL || '' },
        { type: 'input', name: 'CLAUDE_BIN', message: 'Claude CLI path (optional, leave blank to auto-detect):', default: existing.CLAUDE_BIN || '' },
      ])
      Object.assign(env, ans)
    } else {
      // 官方登录：清除三方 token，只配置 bin 路径
  ...
  if (backends.includes('codex')) {
    const { codexAuthMethod } = await inquirer.prompt([{
      type: 'list',
      name: 'codexAuthMethod',
      message: 'Codex 认证方式:',
      choices: [
        { name: '官方账号登录 (codex login)', value: 'official' },
        { name: '三方 API Key (OPENAI_API_KEY)', value: 'third-party' },
      ],
      default: existing.OPENAI_API_KEY ? 'third-party' : 'official',
    }])
    if (codexAuthMethod === 'third-party') {
      const ans = await inquirer.prompt([
        { type: 'input', name: 'OPENAI_API_KEY', message: 'OpenAI API Key:', default: existing.OPENAI_API_KEY || '' },
        { type: 'input', name: 'OPENAI_BASE_URL', message: 'OpenAI API Base URL (optional):', default: existing.OPENAI_BASE_URL || '' },
